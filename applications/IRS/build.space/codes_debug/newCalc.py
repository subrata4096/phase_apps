	...
	if(faultyProcLine):
		...
		if(line.find("States:") != -1):
			...
			if((this_LP_correctly_detected) and (num_of_lp_states > 1)):
					...
					continue
			if(num_of_lp_states > 1):
				cases_with_multiple_LP += 1
...
if(total_LP_correctly_detected > 0):
	falsePositive = 1 - float(float(totalFP)/total_LP_correctly_detected)
	percentage_with_more_than_one_LP = 1 - float(float(cases_with_multiple_LP)/total_LP_task_count)
	precision = 1 - float(float(totalFP)/total_LP_correctly_detected)*float(total_false_positive/float(total_LP_correctly_detected))

	print "False positive: " + str(falsePositive)
	print "Cases LP elimination percentage " + str(percentage_with_more_than_one_LP)
