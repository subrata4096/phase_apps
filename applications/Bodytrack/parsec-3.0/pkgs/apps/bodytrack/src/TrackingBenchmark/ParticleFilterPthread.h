...
void ParticleFilterPthread<T>::Exec(threads::thread_cmd_t cmd, threads::thread_rank_t rank)
{
	...
	if (cmd == workers.THREADS_CMD_PARTICLEWEIGHTS) {
		...
		printf("init particle size = %d", (int)(particles->size()));
		ticket = particleTickets.getTicket();
		while(ticket < (int)(particles->size())) {
			//process all elements in work unit
			//MANISH LOOPCHANGE LOOPSKIP init 1
				// int loopchange = (atoi(getenv("LOOPCHANGE2")));
			printf("outside cmd part weight loop, rank = %d, ticket = %d, size = %d\n", rank, ticket, (int)(particles->size()));
			for(i = ticket; i < (int)(particles->size()) && i < ticket + WORKUNIT_SIZE_PARTICLEWEIGHTS; i+=1) {
				// if((loopchange != 1) && ((i % loopchange) >= 0))
				// 	continue;
				(*valid)[i] = CalcWeight(*particles, i, rank);
				printf("in cmd part weight loop, i = %d, rank = %d, ticket = %d\n", i, rank, ticket);
			}
			ticket = particleTickets.getTicket();
		}
