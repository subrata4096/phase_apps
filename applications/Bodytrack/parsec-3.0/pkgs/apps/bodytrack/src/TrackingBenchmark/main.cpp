...
{
	clock_t manish_Clock = clock();
	cout << endl << "Running Single Threaded" << endl << endl;
	init_num_particles = particles;
	...
	{	cout << "Processing frame " << i << endl;
        ...
        // uint64_t final_cycles = rdtsc();
        // uint64_t difference_cycles = final_cycles - init_cycles;
  ...
  //       printf("\n\n\nDifference is %d\n", final_cycles - init_cycles);
    ...
    printf("overall time taken: %f\n", (double)(manish_Clock)/CLOCKS_PER_SEC); // in seconds
    printf("Frames %d\n", frames); // in seconds
...
int main(int argc, char **argv)
{
	...
	whileLoopCounter = 0;
	phase_manish = atoi(getenv("PHASETOAPPROXIMATE"));
	total_phases_manish = atoi(getenv("TOTALPHASES"));
	total_iterations_manish = atoi(getenv("TOTALITERATIONS"));
