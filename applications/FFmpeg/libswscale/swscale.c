...
{
    int i;
    int loopchange = atoi(getenv("LOOPCHANGE_H815SCALE"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_H815SCALE"));
    clock_t manish_Clock;
    manish_Clock = clock();
    for (i = 0; i < dstW; i+=loopchange) {
        if((loopIterSkip != 1) && ((i % loopIterSkip) == 0))
            continue;
        int j;
        int srcPos = filterPos[i];
        int val    = 0;
        //Possibly change this loop to += 2
        for (j = 0; j < filterSize; j++) {
    ...
    // printf("Time taken by h815scale = %f\n",
    //            time_taken);
                   ...
                   int srcSliceH, uint8_t *dst[], int dstStride[])
{
    ...
    for (; dstY < dstH; dstY++) {
        ...
        //150 -> 4800 MANISH
        int loopchange = atoi(getenv("LOOPCHANGE_SWSCALE"));
        int loopIterSkip = atoi(getenv("LOOPITERSKIP_SWSCALE"));
        clock_t manish_Clock;
        manish_Clock = clock();
        if (posY < lastLumSrcY + 1) {
            for (i = lumStart; i < lumEnd; i+=loopchange)
            {
            if((loopIterSkip != 1) && ((i % loopIterSkip) == 0))
                continue;
        ...
        // printf("Time taken by swscale = %f\n",
        //            time_taken);
