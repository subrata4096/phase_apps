...
static int lum_h_scale(SwsContext *c, SwsFilterDescriptor *desc, int sliceY, int sliceH)
{
    ...
    //4700 -> 84000
    int loopchange = atoi(getenv("LOOPCHANGE_HSCALE"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_HSCALE"));
    clock_t manish_Clock;
    manish_Clock = clock();
    for (i = 0; i < sliceH; i+=loopchange) {
