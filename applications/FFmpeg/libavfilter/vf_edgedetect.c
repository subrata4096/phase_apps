                          ...
                          const uint8_t *src, int src_linesize)
{
    ...
    int loopchange = atoi(getenv("LOOPCHANGE_GAUSSIANBLUR"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_GAUSSIANBLUR"));  
    for (j = 2; j < h - 2; j++) {
        if((loopIterSkip != 1) && ((j % loopIterSkip) == 0))
            continue;
        dst[0] = src[0];
        dst[1] = src[1];
        for (i = 2; i < w - 2; i+=loopchange) {
    ...
    printf("Time taken by gaussianblur = %f\n",
               time_taken);
                  ...
                  const uint8_t *src, int src_linesize)
{
    ...
    int loopchange = atoi(getenv("LOOPCHANGE_SOBEL"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_SOBEL"));  
    for (j = 1; j < h - 1; j+= loopchange) {
        if((loopIterSkip != 1) && ((j % loopIterSkip) == 0))
            continue;
        dst += dst_linesize;
        dir += dir_linesize;
