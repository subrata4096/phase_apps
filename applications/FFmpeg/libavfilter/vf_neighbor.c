...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    int loopchange = atoi(getenv("LOOPCHANGE_NEIGHBOUR"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_NEIGHBOUR"));

    int all_phase_change = (getenv("LOOPITERSKIP_CONVOLVE")) ? atoi(getenv("LOOPITERSKIP_CONVOLVE")) : 0;

    loopchange = (CHECKPHASE()) ? loopchange : 1;
    loopIterSkip = CHECKPHASE() ? loopIterSkip : 1;

    printf("NeighbourPhase = %d, Frame = %d, loopskip1 = %d, loopchange1 = %d\n",phase_manish, frame_manish, loopIterSkip, loopchange);
    ...
    for (plane = 0; plane < s->nb_planes; plane++) {
        ...
        if (threshold) {
            ...
            //101 -> 218160, change this to +=2? MANISH
            for (y = 0; y < height; y+=loopchange) {
                if((loopIterSkip != 1) && ((y % loopIterSkip) == 0))
                    continue;
    ...
    // printf("%f time NEIGHBOUR\n", )        

    printf("Time taken by deflate = %f\n",
               time_taken);

    // printf("Deflate block\n");
