                  ...
                  int w, int h, int radius, int power, uint8_t *temp[2], int pixsize)
{
    ...
    //324 -> 414720 (INIT 100) MANISH
    int loopchange = atoi(getenv("LOOPCHANGE_BOXBLUR"));
    int loopIterSkip = atoi(getenv("LOOPITERSKIP_BOXBLUR"));
    ...
    printf("BoxBlurPhase = %d, Frame = %d, loopskip1 = %d, loopchange1 = %d\n",phase_manish, frame_manish, loopIterSkip, loopchange);
    ...
    for (x = 0; x < w; x+=loopchange)
    {
        if((loopIterSkip != 1) && ((x % loopIterSkip) == 0))
            continue;
