...
/* since we have already tested SetBoxValues above, use SetValues here */
   ...
   if ((solver_id > -1) && (solver_id < 3))
   {
     // printf("my 1");
      ...
      if (solver_id == 0)
      {
        ...
        clock_t t5 = clock();
         ...
         HYPRE_PCGSetPrecond( par_solver,
                             ...
                             par_precond );
      clock_t t6 = clock();
      double d6 = ((double)(t6 - t5))/CLOCKS_PER_SEC;
      //                 printf("\n---- time 6: %e \n", d6);
      }
      else if (solver_id == 1)
      {
      ...
      clock_t t9 = clock();
      double d9 = ((double)(t9 - t7))/CLOCKS_PER_SEC;
                             //printf("\n---- time 9: %e \n", d9);
      hypre_EndTiming(time_index);
      ...
      HYPRE_PCGSolve( par_solver, (HYPRE_Matrix) par_A,
                      (HYPRE_Vector) par_b, (HYPRE_Vector) par_x );
      clock_t t12 = clock();
      double d12 = ((double)(t12 - t11))/CLOCKS_PER_SEC;
      //                 //printf("\n---- time 12: %e \n", d12);
      hypre_EndTiming(time_index);
      ...
      clock_t t8 = clock();
      double d8 = ((double)(t8 - t7))/CLOCKS_PER_SEC;
                 //printf("\n----%d time 8: %e \n", myid, d8);
   ...
   if (myid == 0 )
   {
	...
	(system_size*(double)num_iterations/ wall_time));
      ...
      printf("Solve phase walltime = %e\n", wall_time);
      printf("\n");

      //subrata: results from un-approximated data
      printf ("\n ***  Original un-approximated results for 3 x 3 x 3 = 27 procs ***** \n");
      printf("Iterations ( un-approximated ) = 9 \n");
      printf("Final Relative Residual Norm ( un-approximated ) = 4.439399e-07\n");
      printf ("\nSystem Size * Iterations / Solve Phase Time ( un-approximated ) : 6.707005e+06 \n");
     
      printf("\n \n");
      //subrata
      double qos_degradation = 100 * (double)(4.439399e-07 - final_res_norm)/4.439399e-07;
      double speedup_percentage = 100 * (double)((system_size*(double)num_iterations/ wall_time) - 6.707005e+06)/6.707005e+06; // in computation
      double solvephase_time_reduction_percentage = 100 * (double)(double)(1.855016e-02 - wall_time)/1.855016e-02; //reduction in time
      //
      //
      double speedup = (double)(6.707005e+06)/((system_size*(double)num_iterations/ wall_time)); // in computation
      double solvephase_speedup = (double)(1.855016e-02)/wall_time; //reduction in time
      printf("QoS degradation percentage = %e\n", qos_degradation);
      
      printf("Computation speedup = %e\n", speedup);
      printf("Solve phase speedup = %e\n", solvephase_speedup);
      
      printf("Computation speedup percentage = %e\n", speedup_percentage);
      printf("Solve phase time reduction percentage = %e\n", solvephase_time_reduction_percentage);
   }
   ...
   if(myid == 3)
   {
	   ...
	   //string hostStr = string(hostname);
	   //printf("\n\n\n Log in to rank 3 at : %s  \n\n", name);
	   //sleep(360000);
	   //while(1);
   }
   ...
   //           printf("\n----%d Total time taken by AMG: %e \n",myid, total_time);
   if(myid == 0)
   {
           double total_time = ((double)(t2 - t1))/CLOCKS_PER_SEC;
           printf("\n---- Total time taken by AMG: %e \n", total_time);
            struct rusage data;
