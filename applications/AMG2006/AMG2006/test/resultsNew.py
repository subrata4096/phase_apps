	...
	if(faultyProcLine):
		...
		if(line.find("States:") != -1):
			...
			if(this_LP_correctly_detected):
				print "detected " + " count= " + str(num_of_LP)
				fp = float(1 - num_of_LP)/for_num_proc
				totalFP += fp
			else:
				print "NOT DETECTED"
...
print "Total LP task: " + str(total_LP_task_count) + " total correctly detected: " +  str(total_LP_correctly_detected) 
