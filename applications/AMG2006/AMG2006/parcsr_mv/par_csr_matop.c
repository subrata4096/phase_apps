 ...
 *--------------------------------------------------------------------------*/
   ...
   int loop_skip=1;   //subrata
   /*
   int loop_subskip=1;   //subrata
   bool loop_skip_is_int = true;
   const char* env_skip = getenv("SKIP");
   if(env_skip)
   {
     float skip_val = atof(env_skip);
     if(skip_val > 1)
     {
         loop_skip = int(skip_val);
	 loop_skip_is_int = true;
     }
     else
     {
	     loop_subskip = 1/skip_val;
	     loop_skip = 1;
	     loop_skip_is_int = false; 
     }
   }
   */

   clock_t t_begin, t_end;  //subrata
   t_begin = clock();  //subrata
   for (i=0; i < num_rows_B_ext /*&& ((i+loop_skip) < num_rows_B_ext )*/; i += loop_skip)
   {
	   /*
	   if(my_id == 0)
	   {
	   ...
	   if((loop_skip_is_int == false) && (0 == i%loop_subskip )) //subrata
	   {
		    continue;             //subrata
	   }
   ...
   t_end = clock(); //subrata
   //printf("Local compute speedup (par_csr_matop.c:948) = %e\n", (double)(100)/((t_end - t_begin)/CLOCKS_PER_SEC));
   //printf("Local compute speedup (par_csr_matop.c:948) = %e  for skip = %d\n", (1.000000e+04/((double)(t_end - t_begin))), loop_skip);
